<table class="w-full border-collapse text-left">
    ...
    <tbody class="divide-y divide-gray-100">
        @foreach ($this->winnersPaginated as $index => $winner)
            <tr class="winner-item text-black transition-colors hover:bg-gray-50"
                wire:key="table-{{ $winner['raffle_number'] }}">
                <td class="px-6 py-4 font-bold text-blue-600">
                    #{{ ($this->winnersPaginated->currentpage() - 1) * $this->winnersPaginated->perpage() + $loop->index + 1 }}
                </td>
                <td class="px-6 py-4 font-mono">{{ $winner->submission->raffle_number }}</td>
