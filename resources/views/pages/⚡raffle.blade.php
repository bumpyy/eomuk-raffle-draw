...
new class extends Component
{
    ...
    public function exportCsv()
    {
        ...
        return response()->streamDownload(function () use ($winnersExport) {
            ...
            fputcsv($file, ['No', 'Raffle Number', 'Name', 'Email', 'Phone', 'Prize']);

            foreach ($winnersExport as $index => $winner) {
                fputcsv($file, [$index + 1, $winner['raffle_number'], $winner['name'], $winner['email'], $winner['phone'], $winner['prize']->label()]);
            }
    ...
    public function exportExcel()
    {
        ...
        $filename = "raffle_winners_" . now()->format('Ymd_His') . '.xlsx';
...
<div class="relative mt-20 flex min-h-screen flex-col items-center py-12 font-sans" x-cloak x-data="{
    ...
    <div class="mb-12 flex h-16 flex-col items-center">
        @if (count($winners) < $prize->targetWinners())
            ...
            @else
            <div class="text-2xl font-black uppercase text-white drop-shadow-sm">SELAMAT KEPADA PARA PEMENANG!</div>
            @endif
    </div>
    ...
    <div class="w-full max-w-7xl px-6">
        <div class="mb-6 flex items-center justify-between border-b-2 border-gray-200 pb-4">
            <div class="flex items-center space-x-2 rounded-lg bg-gray-200 p-1">
                <button class="rounded-md px-3 py-1 text-sm font-bold transition-all" type="button"
                    @click="viewMode = 'grid'"
            ...
            <div class="flex space-x-3">
                <a class="bg-cedea-blue inline-flex items-center rounded-lg px-4 py-2 text-sm font-bold text-white transition-colors"
                    href="{{ route('welcome') }}" target="_self">
                    Back to Main Menu
                </a>

                @if (count($winners) > 0)
                <button
                    class="cursor-pointer rounded-lg bg-green-100 px-4 py-2 text-sm font-bold text-green-700 transition-colors hover:bg-green-200"
                    wire:click="exportCsv">
                    Export CSV
                </button>

                <button
                    class="cursor-pointer rounded-lg bg-emerald-100 px-4 py-2 text-sm font-bold text-emerald-700 transition-colors hover:bg-emerald-200"
                    wire:click="exportExcel">
                    Export Excel
                </button>

                <!-- <a class="bg-cedea-blue inline-flex cursor-pointer items-center rounded-lg px-4 py-2 text-sm font-bold text-white transition-colors" href="{{ route('export.pdf', ['prize' => $prize->value]) }}" target="_blank">
                    Export PDF
                </a> -->
                ...
                <button
                    class="cursor-pointer rounded-lg bg-red-100 px-4 py-2 text-sm font-bold text-red-600 transition-colors hover:bg-red-200"
